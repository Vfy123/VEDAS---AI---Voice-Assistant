...
if __name__ == "__main__":
    ...
    if SSL_CERT.exists() and SSL_KEY.exists():
        uvicorn.run("vedas_server:app", host="0.0.0.0", port=8000, ssl_certfile=str(SSL_CERT), ssl_keyfile=str(SSL_KEY), reload=False)
    else:
        uvicorn.run("vedas_server:app", host="0.0.0.0", port=8000, reload=False)
